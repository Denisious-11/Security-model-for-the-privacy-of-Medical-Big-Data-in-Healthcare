#p array which contains all the  Sub-keys (18 subkeys)
p = [
...
# s is the substitution boxes (there are total 4 substitution boxes)
s = [
      [
...
#calculate function is applicable for each 16 rounds
#(previous round plain text + the current round subkeys)
def calculate(L):
    temp = s[0][L >> 24]
...
def encrypt(data):
        L = data>>32
        R = data & 0xffffffff
        for i in range(0,16):# calling calculate function for 16 rounds
                L = L^p[i]
                L1 = calculate(L) #function call
                R = R^calculate(L1)
                L,R = R,L
        L,R = R,L
        L = L^p[17]
...
def decrypt(data):
    L = data >> 32
    R = data & 0xffffffff
    for i in range(17, 1, -1):# calling calculate function for 16 rounds (here subkeys are used in reverse)
        L = p[i]^L
        L1 = calculate(L)#functoion call
        R = R^calculate(L1)
        L,R = R,L
